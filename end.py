...
class KNN_SHC:
    ...
    def compute_err(self,Y_L, Y_L_pred):
        """计算经验损失 (Err)"""
    ...
    def compute_sse(self):
        ...
        for i in range(len(self.centroids)):
            cluster_points = np.vstack((X_L, X_U))[self.labels == i]
            sse += np.sum(np.linalg.norm(cluster_points - self.centroids[i], axis=1) ** 2)
        return sse

    def total_loss(self, Err_t, SSE_t):
        """计算总损失函数 (L)"""
        Err_value = self.compute_err(self.Y_L, self.Y_L_Pre)
        ...
        # 计算总损失
        total_loss_value = self.lambda_param * Err_prime_j + (1 - lambda_param) * SSE_prime_j
    ...
    def iterative_clustering(self):
        ...
        while 1 == 1:
            ...
            for cluster_id, cluster_points in list(self.clusters.items()):
                ...
                filtered_labels = unique_labels[valid_indices]
                filtered_counts = counts[valid_indices]
                print(f"{cluster_id}簇内全部标签 {combined_labels}")
                print(f"{cluster_id}簇内不同标签 {filtered_labels} :{filtered_counts}")
                # 如果簇内有多个不同的标签
                if len(filtered_labels) > 0:
                    ...
                    for label in filtered_labels:
                        ...
                        if label != majority_label:
                            ...
                            if len(new_cluster_points) > 0:
                                ...
                                print(f"样本为:{new_cluster_points}")
                    ...
                    for x in cluster_points:
                        ...
                        if np.sum(knn_neighbors_labels == cluster_id) <= 3:
                            # 根据近邻情况将x转移到其它簇
                            print(f"将要将样本从簇 {cluster_id} 转移到簇 {new_cluster_id}")
                            cluster_probabilities = self.calculate_cluster_probabilities(x)
            ...
            print("不相等标签个数是：" + str(num_errors))
            current_loss = self.total_loss(Err_t, SSE_t)
            print("损失函数：" + str(current_loss))
    ...
    def final_clustering(self):
        ...
        acc = self.calculate_label_accuracy()
        print(f"最终分簇后:簇个数: {len(self.clusters)},簇类别: {self.labels.items()}")
        print(f"分配簇的ACC为: {acc}")
    ...
    def calculate_label_accuracy(self):
        ...
        for cluster_id, cluster_points in list(self.clusters.items()):
            ...
            if len(valid_labels) == 0:
                print(f"当前簇内没有有效的标签: {cluster_id}")
                continue
...
print("Unique labels in Y_L:", np.unique(Y_L))
print("Label counts:", np.bincount(Y_L.astype(int)))
# 初始化KNN-SHC算法并运行
